...
function recomputeStats() {
    ...
    if (typeof applyLootAffixAttributes === 'function') applyLootAffixAttributes(p, d);   // 🎲 品質詞綴：六維提前計入
    ...
    if (p.cls === 'knight') {
        ...
        d.meleeDmg += Math.floor(p.lv / 3);    // ⚖️ 平衡測試：騎士每提升3級 近距離傷害+1（原每4級）
        ...
        d.rangedDmg += Math.floor(p.lv / 3);   // ⚖️ 平衡測試：妖精每提升3級 遠距離傷害+1（原每4級）
        ...
        d.er += Math.floor(p.lv / 4);          // 每提升4 ER+1
        d.meleeDmg += Math.floor(p.lv / 4);    // ⚖️ 平衡測試：黑暗妖精每提升4級 近距離傷害+1（原每5級）
        ...
        d.extraDmg += Math.floor(p.lv / 4);    // ⚖️ 平衡測試：王族每提升4級 額外傷害+1（原每5級）
...
d.mr += (baseMr + bonusMr);
    ...
    if (typeof applyLootAffixCombat === 'function') applyLootAffixCombat(p, d);   // 🎲 品質詞綴：戰鬥/恢復/抗性/HPMP%/速度
