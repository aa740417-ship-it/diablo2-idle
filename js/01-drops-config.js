...
function itemSig(it) { let _ams = Math.max(1, Math.min(3, Math.floor(Number(it.attrMagicStar) || 1))); let _la = Array.isArray(it.lootAff) ? it.lootAff.map(a => a.k + ':' + a.v).join(',') : ''; return it.id + '|' + (it.en || 0) + '|' + (it.bless === true ? 'B' : (it.bless ? 'C' : 0)) + '|' + (it.anc === true ? 'A' : (it.anc || 0)) + '|' + (it.attr || '') + '|' + (it.seteff || '') + '|Q:' + (it.lootQ || 'white') + '|A:' + _la + (it.attrMagic ? '|' + it.attrMagic + (_ams > 1 ? '@' + _ams : '') : ''); }   // 來源、uid、鎖定與廢品旗標不入鍵；同能力物品不論來源皆可疊加。
...
//      一般怪 HP×[3/5]、頭目 HP×[5/10]、AC 固定額外降低（頭目−20/一般−10）、MR 強化、命中×[1.5/2]、
//      額外減傷＝floor(等級/[5/4])（頭目再+[3/5]）、
//      經驗×[5/10]、金錢×[5/10]；一般怪傷害×[2/3]，頭目傷害×[2.5/4]（含技能與持續傷害）；
