...
function applySherineBuff(idx) {
    ...
    if (sherineWorldActive() && !isSiegeArea(mapState.current) && _m.race !== '血盟') {
        ...
        // ⚖️ 2026-08 平衡測試：一般怪維持 HP×3/×5；頭目改為席琳×5、瘋狂席琳×10，延長王戰而不是只堆防禦。
        let _hpMul = _m.boss ? (_mad ? 10 : 5) : (_mad ? 5 : 3);
        _m.hp = Math.floor(_m.hp * _hpMul); _m.curHp = _m.hp;
        _m.ac = (_m.ac || 0) - (_m.boss ? 20 : 10);                    // 🔮 席琳 AC：維持固定值，避免物理命中被過度壓低
        ...
        // ⚖️ 物理公平性：原本等級/3 的席琳 DR 對物理職過重，改為一般/5、瘋狂/4；頭目只額外 +3/+5 DR。
        _m.dr = (_m.dr || 0) + Math.floor((_m.lv || 1) / (_mad ? 4 : 5)) + (_m.boss ? (_mad ? 5 : 3) : 0);
        _m._sherine = true;
        // ⚖️ 頭目傷害：席琳×2.5、瘋狂×4；一般怪仍維持×2/×3。
        _m._sherineDmgMult = _m.boss ? (_mad ? 4 : 2.5) : (_mad ? 3 : 2);
