...
// 📋 自訂版：NPC 對話視窗固定提供「✕」返回村莊列表；手機捲到任何位置都能關閉。
let _npcCloseObserver = null;
function _installNpcCloseButton(contentDiv) {
    if (!contentDiv) contentDiv = document.getElementById('interaction-content');
    if (!contentDiv) return;
    let btn = contentDiv.querySelector('#npc-dialog-close-btn');
    if (!btn) {
        btn = document.createElement('button');
        btn.id = 'npc-dialog-close-btn';
        btn.type = 'button';
        btn.setAttribute('aria-label', '關閉 NPC 對話');
        btn.title = '返回村莊';
        btn.textContent = '✕';
        btn.onclick = function(e) { e.preventDefault(); e.stopPropagation(); closeNpcInteraction(); };
        btn.style.cssText = 'position:sticky;top:8px;z-index:99999;display:block;margin:0 0 8px auto;width:44px;height:44px;min-width:44px;min-height:44px;border:1px solid rgba(148,163,184,.65);border-radius:999px;background:rgba(15,23,42,.96);color:#fff;font-size:26px;line-height:40px;text-align:center;box-shadow:0 4px 14px rgba(0,0,0,.45);touch-action:manipulation;';
        contentDiv.insertBefore(btn, contentDiv.firstChild);
    }
    // NPC 內頁常會因製作/購買而 innerHTML 重繪；監看後自動把叉叉補回來。
    if (_npcCloseObserver) _npcCloseObserver.disconnect();
    _npcCloseObserver = new MutationObserver(function() {
        if (!contentDiv.querySelector('#npc-dialog-close-btn') && !document.getElementById('town-interaction-container').classList.contains('hidden')) {
            _installNpcCloseButton(contentDiv);
        }
    });
    _npcCloseObserver.observe(contentDiv, { childList:true });
}
...
function closeNpcInteraction() {
    if (_npcCloseObserver) { try { _npcCloseObserver.disconnect(); } catch (e) {} _npcCloseObserver = null; }
...
function renderTownNPCMap(townId) {
    ...
    // 📋 自訂版：城鎮 NPC 改成列表式，不再把 NPC 人物散佈在地圖畫面上。
    // 保留原本 NPC 點擊功能；塔/裂痕入口與玩家收購 NPC 也一併列入。
    map.style.backgroundImage = 'none';
    // 📱 手機版固定 NPC 清單高度，讓它成為真正可捲動區，不再被下方戰鬥日誌截斷。
    let _npcListH = Math.max(300, Math.min(430, Math.floor((window.innerHeight || 720) * 0.50)));
    map.style.height = _npcListH + 'px';
    map.style.minHeight = _npcListH + 'px';
    map.style.maxHeight = _npcListH + 'px';
    map.style.flex = '0 0 ' + _npcListH + 'px';
    map.style.overflowX = 'hidden';
    map.style.overflowY = 'scroll';
    map.style.webkitOverflowScrolling = 'touch';
    map.style.touchAction = 'pan-y';
    map.style.overscrollBehavior = 'contain';
    map.style.display = 'flex';
    map.style.flexDirection = 'column';
    map.style.gap = '8px';
    map.style.padding = '10px 10px 120px 10px';
    map.style.scrollPaddingBottom = '120px';
    map.style.boxSizing = 'border-box';
    // 某些 Android 瀏覽器在巢狀面板裡不會把 touchmove 交給 overflow 區塊，手動補捲動。
    let _npcTouchY = null, _npcTouchTop = 0, _npcTouchMoved = false;
    map.ontouchstart = function(ev) {
        if (!ev.touches || !ev.touches.length) return;
        _npcTouchY = ev.touches[0].clientY;
        _npcTouchTop = map.scrollTop;
        _npcTouchMoved = false;
    };
    map.ontouchmove = function(ev) {
        if (_npcTouchY == null || !ev.touches || !ev.touches.length) return;
        let dy = _npcTouchY - ev.touches[0].clientY;
        if (Math.abs(dy) > 4) {
            _npcTouchMoved = true;
            map.scrollTop = _npcTouchTop + dy;
            if (ev.cancelable) ev.preventDefault();
        }
    };
    map.ontouchend = function() { _npcTouchY = null; setTimeout(() => { _npcTouchMoved = false; }, 0); };
    vis.forEach(npc => {
        let row = document.createElement('button');
        row.type = 'button';
        row.style.cssText = 'width:100%;display:flex;flex:0 0 auto;align-items:center;justify-content:space-between;gap:12px;padding:12px 14px;border:1px solid rgba(148,163,184,.35);border-radius:8px;background:rgba(15,23,42,.88);color:#e2e8f0;text-align:left;min-height:54px;';
        let left = document.createElement('span');
        left.style.cssText = 'display:flex;flex-direction:column;gap:3px;min-width:0;';
        let name = document.createElement('span');
        name.style.cssText = 'font-weight:700;color:#fcd34d;font-size:16px;';
        name.textContent = npc.n || 'NPC';
        let title = document.createElement('span');
        title.style.cssText = 'font-size:13px;color:#94a3b8;';
        title.textContent = npc.title || (npc._wanderer ? '玩家收購' : '');
        left.appendChild(name);
        if (title.textContent) left.appendChild(title);
        let arrow = document.createElement('span');
        arrow.style.cssText = 'font-size:22px;color:#94a3b8;flex:0 0 auto;';
        arrow.textContent = '›';
        row.appendChild(left); row.appendChild(arrow);
        row.onclick = () => {
            if (_npcTouchMoved) return;
            if (npc._wanderer) { if (typeof openWanderingBuyerDialog === 'function') openWanderingBuyerDialog(npc.id); return; }
            if (npc._float === 'pride') { openTownFloatWindow('傲慢之塔', '排名挑戰', renderPrideEntrance); return; }
            if (npc._float === 'rift') { openTownFloatWindow('時空裂痕', '限時挑戰', renderRiftEntrance); return; }
            interactNPC(npc.id, townId);
        };
        map.appendChild(row);
    });
    return;
...
function openTownFloatWindow(name, title, renderFn) {
    ...
    _installNpcCloseButton(content);
}
