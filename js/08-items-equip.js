...
// ===== 🎲 裝備品質＋隨機詞綴（測試版） =====
// 白色 70%：0 詞綴；藍色 25%：1 詞綴；暗金 5%：3 詞綴。
const LOOT_QUALITY = {
    white: { n:'白色', cls:'text-white', affixes:0 },
    blue:  { n:'藍色', cls:'text-blue-400', affixes:1 },
    unique:{ n:'暗金', cls:'text-amber-400', affixes:3 }
};
const LOOT_AFFIX_POOL = [
    ['str','力量',1,5],['dex','敏捷',1,5],['con','體質',1,5],['int','智力',1,5],['wis','精神',1,5],['cha','魅力',1,5],
    ['meleeDmg','近距離傷害',5,10],['rangedDmg','遠距離傷害',5,10],['magicDmg','魔法傷害',5,10],
    ['meleeHit','近距離命中',5,10],['rangedHit','遠距離命中',5,10],['magicHit','魔法命中',5,10],
    ['meleeCrit','近距離爆擊率',5,10],['rangedCrit','遠距離爆擊率',5,10],['magicCrit','魔法爆擊率',5,10],
    ['meleeCritDmg','近距離爆擊傷害',15,30],['rangedCritDmg','遠距離爆擊傷害',15,30],['magicCritDmg','魔法爆擊傷害',15,30],
    ['hpR','HP自然恢復',5,10],['mpR','MP自然恢復',5,10],['hitHp','擊中回血',5,10],['hitMp','擊中回魔',5,10],
    ['atkSpdPct','攻擊速度',5,20],['castSpdPct','施法速度',5,20],
    ['bossDmgPct','頭目傷害',10,20],['normalDmgPct','一般怪物傷害',10,20],
    ['resFire','火屬性抗性',5,10],['resWater','水屬性抗性',5,10],['resEarth','地屬性抗性',5,10],['resWind','風屬性抗性',5,10],
...
function applyLootAffixAttributes(p,d) {
    ...
    for (let k in p.eq) { let e=p.eq[k]; if(!e || !Array.isArray(e.lootAff)) continue; for (let a of e.lootAff) {
        if (['str','dex','con','int','wis','cha'].includes(a.k)) d[a.k] += Number(a.v)||0;
    }}
}
function applyLootAffixCombat(p,d) {
    if (!p || !p.eq) return;
    let hpPct=0, mpPct=0, castPct=0;
    d.lootHitHp=0; d.lootHitMp=0; d.lootBossDmgPct=0; d.lootNormalDmgPct=0;
    for (let k in p.eq) { let e=p.eq[k]; if(!e || !Array.isArray(e.lootAff)) continue; for (let a of e.lootAff) { let v=Number(a.v)||0;
        if (['meleeDmg','rangedDmg','magicDmg','meleeHit','rangedHit','magicHit','meleeCrit','rangedCrit','magicCrit','meleeCritDmg','rangedCritDmg','magicCritDmg','hpR','mpR','resFire','resWater','resEarth','resWind','mr'].includes(a.k)) d[a.k]+=v;
        // 舊測試版存檔相容：若已有全類爆擊詞綴，仍照舊生效，但新掉落不再抽到。
        else if(a.k==='allCrit'){ d.meleeCrit+=v; d.rangedCrit+=v; d.magicCrit+=v; }
        else if(a.k==='allCritDmg'){ d.meleeCritDmg+=v; d.rangedCritDmg+=v; d.magicCritDmg+=v; }
        else if(a.k==='atkSpdPct') d.atkSpdPct+=v;
        else if(a.k==='castSpdPct') castPct+=v;
        else if(a.k==='hpPct') hpPct+=v; else if(a.k==='mpPct') mpPct+=v;
        else if(a.k==='defense') d.ac-=v;
        else if(a.k==='hitHp') d.lootHitHp+=v; else if(a.k==='hitMp') d.lootHitMp+=v;
        else if(a.k==='bossDmgPct') d.lootBossDmgPct+=v; else if(a.k==='normalDmgPct') d.lootNormalDmgPct+=v;
    }}
    if (hpPct) p.mhp = Math.floor(p.mhp * (1 + hpPct/100));
    if (mpPct) p.mmp = Math.floor(p.mmp * (1 + mpPct/100));
    if (castPct) { if (d.castLock) d.castLock=Math.max(1,d.castLock/(1+castPct/100)); if(d.supportCastLock) d.supportCastLock=Math.max(1,d.supportCastLock/(1+castPct/100)); }
}
...
function gainItem(id, cnt=1, silent=false, forceNormal=false, affixOld=false, deferUi=false, fixedAffixes=null, blessRate=null) {   // ⚠️ 第 7 參供既定詞綴來源；第 8 參供製作等來源指定祝福率
    ...
    let _probe = { id: id, en: _tEn, bless: bless, anc: anc, attr: attr, seteff: seteff, lootQ: lootQ, lootAff: lootAff };
...
function batchUseItem(u) {
    ...
    if (d.eff === 'panacea') {
        ...
        let rawP = prompt(`要使用幾瓶 ${d.n}？（持有 ${item.cnt} 瓶·${STAT_CN[st]}距自然上限 ${remainStat}·本次最多 ${maxN} 瓶）`, maxN);
        if (rawP === null) return;
        ...
        calcStats();
        logSys(`使用了 <span class="${d.c || 'text-pink-300'} font-bold">${d.n}</span> ×${nP}，${STAT_CN[st]} 永久 +${nP}！（萬能藥總使用 ${player.panaceaUsed} 瓶）`);
...
function useItem(u, silent = false) {
    ...
    if (d.type === 'pot' || d.eff === 'poly' || d.eff === 'reset' || d.eff === 'magicbarrier' || d.eff === 'teleport_scroll' || d.eff === 'panacea') {   // 變形卷軸(eff:poly)、回憶蠟燭(eff:reset)、魔法卷軸(eff:magicbarrier)、瞬間移動卷軸(eff:teleport_scroll)亦走此消耗品分支
        ...
        if (item.id.includes('potion_heal') || item.id === 'potion_strong' || item.id === 'potion_ult') {
            ...
            if(!silent) logSys(`使用了 ${d.n}，${STAT_CN[st]} 永久 +1！（萬能藥總使用 ${player.panaceaUsed} 瓶）`);
