...
function _enemyPhysicalAttackInner(mob, idx, stunChance = 0, atkDmg = null, atkDb = null, isBasicAttack = false) {   // atkDmg/atkDb：連擊技覆寫骰子/加值（如鐮刀劍氣斬 9×3D70+99，與一般攻擊不同）
    ...
    if (hit) {
        ...
        if (mob._sherine) totalDmg = Math.floor(totalDmg * (mob._sherineDmgMult || (mob._sherineMad ? 3 : 2)));   // 🔮 席琳的世界：怪物一般攻擊傷害 ×2（瘋狂×3）
        ...
        if (!player.dead && mob.onHitPoison && !player.d.immPoison) {
            ...
            if (Math.random() * 100 < _ohc) {
                if (playerStatusResisted('poison')) { logCombat('<span class="text-sky-300 font-bold">你抵抗了中毒！</span>', 'magic'); }
                    ...
                    let _ohD = Math.floor(((mob._sherine ? (mob._sherineDmgMult || (mob._sherineMad ? 3 : 2)) : 1) * (mob._grace ? 2 : 1)) * (_ohp.d || 10) * mobRageDmgMult(mob));   // 席琳／頭目狂暴倍率（比照施法中毒）
...
function _enemyAttackAllyInner(mob, ally, isBasicAttack = false) {
    ...
    if (mob._sherine) totalDmg = Math.floor(totalDmg * (mob._sherineDmgMult || (mob._sherineMad ? 3 : 2)));
    ...
    if (ally.curHp > 0 && mob.onHitPoison && !d.immPoison) {
        ...
        if (Math.random() * 100 < _ohc) {
            if (allyStatusResisted(ally, 'poison')) logCombat(`<span class="text-sky-300 font-bold">協力·${ally._allyName} 抵抗了中毒！</span>`, 'magic');
                ...
                let _ohD = Math.floor(((mob._sherine ? (mob._sherineDmgMult || (mob._sherineMad ? 3 : 2)) : 1) * (mob._grace ? 2 : 1)) * (_ohp.d || 10) * mobRageDmgMult(mob));
...
function _applyMobMagicToAllyInner(mob, sk, ally) {
    ...
    let _shMul = (mob._sherine ? (mob._sherineDmgMult || (mob._sherineMad ? 3 : 2)) : 1) * (mob._grace ? 2 : 1);   // 🔮 席琳：傷害/持續傷害倍率
...
function _applyMobMagicInner(mob, sk) {
    ...
    if(sk.type === 'scald') {
        ...
        if(Math.random() * 100 < chance && !player.dead) {
            ...
            let _scD = ((mob._sherine ? (mob._sherineDmgMult || (mob._sherineMad ? 3 : 2)) : 1) * (mob._grace ? 2 : 1)) * (sk.d||100);   // 🔮 席琳的世界：持續傷害×2
    ...
    if(sk.type === 'poison') {
        ...
        if(Math.random() * 100 < chance && !player.dead) {
            ...
            let _poD = Math.floor(((mob._sherine ? (mob._sherineDmgMult || (mob._sherineMad ? 3 : 2)) : 1) * (mob._grace ? 2 : 1)) * sk.d * mobRageDmgMult(mob));   // 🔮 席琳的世界／🔥頭目狂暴：持續傷害倍率
    ...
    if(sk.type === 'burn') {
        ...
        let _buD = Math.floor(((mob._sherine ? (mob._sherineDmgMult || (mob._sherineMad ? 3 : 2)) : 1) * (mob._grace ? 2 : 1)) * sk.d * mobRageDmgMult(mob));   // 🔮 席琳的世界／🔥頭目狂暴：持續傷害倍率
    ...
    if(sk.dmg) {
        ...
        if (mob._sherine) dmg = Math.floor(dmg * (mob._sherineDmgMult || (mob._sherineMad ? 3 : 2)));            // 🔮 席琳的世界：技能最終傷害 ×2（瘋狂×3·增傷）
        ...
        if(sk.sec) {
            ...
            if(sk.sec.type === 'burn') {
                ...
                if(Math.random() * 100 < chance && !player.dead) {
                    ...
                    let _sbD = Math.floor(((mob._sherine ? (mob._sherineDmgMult || (mob._sherineMad ? 3 : 2)) : 1) * (mob._grace ? 2 : 1)) * sk.sec.d * mobRageDmgMult(mob));   // 🔮 席琳的世界／🔥頭目狂暴：持續傷害倍率
            ...
            if(sk.sec.type === 'scald') {
                ...
                if(Math.random() * 100 < chance && !player.dead) {
                    ...
                    let _ssD = Math.floor(((mob._sherine ? (mob._sherineDmgMult || (mob._sherineMad ? 3 : 2)) : 1) * (mob._grace ? 2 : 1)) * sk.sec.d * mobRageDmgMult(mob));   // 🔮 席琳的世界／🔥頭目狂暴：持續傷害倍率
            ...
            if(sk.sec.type === 'poison' && !player.d.immPoison) {   // 潔尼斯戒指免疫中毒
                ...
                if(Math.random() * 100 < chance && !player.dead) {
                    ...
                    let _spD = Math.floor(((mob._sherine ? (mob._sherineDmgMult || (mob._sherineMad ? 3 : 2)) : 1) * (mob._grace ? 2 : 1)) * sk.sec.d * mobRageDmgMult(mob));   // 🔮 席琳的世界／🔥頭目狂暴：持續傷害倍率
...
function pledgeBonusDrop(mob, rate) {
    ...
    if (isEquip) {
        ...
        let en = rollPledgeDropEnhance(d0.safe || 0);
        let _lq = (typeof rollLootQuality === 'function' && !d0.legend) ? rollLootQuality(id) : {q:'white',aff:[]};   // 依物品安定值決定強化等級（🏛️v3.0.83 傳統權重表分流已移除）
        let _jProbe = { id: id, en: en, bless: bless, anc: anc, attr: attr, seteff: false, lootQ:_lq.q, lootAff:_lq.aff };   // 🔧 廢品記憶：血盟/攻城掉寶比照 gainItem，依完整簽章（含強化/祝福/詞綴）自動標記
        invAddOrStack({ id: id, uid: uid(), cnt: 1, en: en, bless: bless, anc: anc, attr: attr, seteff: false, lootQ:_lq.q, lootAff:_lq.aff, lock: false, junk: !!(player.junkPrefs && player.junkPrefs[itemSig(_jProbe)]) });
        ...
        item = { id: id, en: en, bless: bless, anc: anc, attr: attr, lootQ:_lq.q, lootAff:_lq.aff, cnt: 1 };
    } else {
