...
function renderClassicSkillBook(sDiv) {
    ...
    let cells = list.map(entry => {
        ...
        let elementOk = !sk.reqEle || player.cls === 'elf' || player.elfEle === sk.reqEle;
...
function renderSkillSelects() {
    ...
    sortedSkills.forEach(sid => {
        ...
        if(!__granted && sk.reqEle && player.cls !== 'elf' && player.elfEle !== sk.reqEle) isAvail = false;
...
function buildItemDescHTML(item) {
    ...
    if (item && item.lootQ) { let _qc = item.lootQ === 'unique' ? 'text-amber-400' : (item.lootQ === 'blue' ? 'text-blue-400' : 'text-white'); let _ql = item.lootQ === 'unique' ? '暗金' : (item.lootQ === 'blue' ? '藍色' : '白色'); desc = `<span class=\"${_qc} font-bold\">品質：${_ql}</span>` + (Array.isArray(item.lootAff) && item.lootAff.length ? '<br>' + item.lootAff.map(a => `<span class=\"${_qc}\">✦ ${typeof lootAffixText === 'function' ? lootAffixText(a) : (a.n+' +'+a.v)}</span>`).join('<br>') : '') + (desc ? '<br>' + desc : ''); }
...
function getAutoSellRules() {
    if (!player.autoSellRules) player.autoSellRules = {
        ...
        quality: { white:false, blue:false },
    ...
    if (r.quality.white == null) r.quality.white = false;
    if (r.quality.blue == null) r.quality.blue = false;
    if (!r.equip) r.equip = {};
...
window.useSkillBookFromModal = function(uid){
    ...
    try {
        ...
        /* === elf four elements real learning fix v1 === */
/* 妖精屬性技能 */
        if(
            sk.reqEle &&
            player.cls !== 'elf' &&
