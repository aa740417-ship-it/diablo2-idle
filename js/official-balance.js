...
(function () {
    ...
    const MONSTER_BALANCE = {
        ...
        // OB29：傲慢之塔 2~10 樓
        '變種蛇女':          { exp: 1.06, gold: 1.02, drop: 1.03 },
        '變種楊果里恩':      { exp: 1.08, gold: 1.03, drop: 1.04 },
        '梅杜莎':            { exp: 1.10, gold: 1.04, drop: 1.05 },
        '奇美拉':            { exp: 1.12, gold: 1.05, drop: 1.06 },
        // 亞力安沿用 OB18：{ exp:1.15, gold:1.06, drop:1.07 }
        '扭曲的潔尼斯女王':  { exp: 1.42, gold: 1.20, drop: 1.28 },

        // OB30：傲慢之塔 11~20樓
        // 思克巴不在此加入，避免它在其他已平衡地圖也同步被改動。
        '魔狼':              { exp: 1.08, gold: 1.03, drop: 1.04 },
        '邪惡密密':          { exp: 1.10, gold: 1.04, drop: 1.05 },
        '邪惡多眼怪':        { exp: 1.12, gold: 1.05, drop: 1.06 },
        '死亡之劍':          { exp: 1.10, gold: 1.04, drop: 1.05 },
        '不幸的幻象眼魔':    { exp: 1.45, gold: 1.20, drop: 1.30 },

        // OB31：傲慢之塔 21~30樓
        '恐怖的火炎蛋':      { exp: 1.08, gold: 1.03, drop: 1.04 },
        '恐怖夢魘':          { exp: 1.10, gold: 1.04, drop: 1.05 },
        '恐怖的地獄犬':      { exp: 1.12, gold: 1.05, drop: 1.06 },
        '小惡魔':            { exp: 1.13, gold: 1.06, drop: 1.06 },
        '恐怖的伊弗利特':    { exp: 1.18, gold: 1.08, drop: 1.08 },
        '恐怖的吸血鬼':      { exp: 1.48, gold: 1.22, drop: 1.30 },

        // OB32：傲慢之塔 31~40樓
        '殘暴的骷髏斧兵':    { exp: 1.10, gold: 1.03, drop: 1.04 },
        '殘暴的食屍鬼':      { exp: 1.12, gold: 1.04, drop: 1.05 },
        '殘暴的骷髏槍兵':    { exp: 1.10, gold: 1.03, drop: 1.04 },
        '殘暴的史巴托':      { exp: 1.14, gold: 1.05, drop: 1.06 },
        '殘暴的骷髏神射手':  { exp: 1.16, gold: 1.06, drop: 1.07 },
        '殘暴的骷髏鬥士':    { exp: 1.18, gold: 1.07, drop: 1.08 },

        // 原始金幣 9000~16200，故金幣倍率刻意壓低。
        '死亡的殭屍王':      { exp: 1.52, gold: 0.72, drop: 1.28 },

        // OB33：傲慢之塔 41~50樓
        // 幼龍／骨龍可能在其他內容出現，避免建立全域名稱倍率。
        '火焰之靈魂(紅)':    { exp: 1.10, gold: 1.03, drop: 1.04 },
        '火焰之靈魂(藍)':    { exp: 1.12, gold: 1.04, drop: 1.05 },
        '恐怖的鋼鐵高崙':    { exp: 1.15, gold: 1.05, drop: 1.06 },
        '火焰之魔法師':      { exp: 1.16, gold: 1.06, drop: 1.07 },

        // 原始金幣 10000~20000，故額外壓低金幣倍率。
        '地獄的黑豹':        { exp: 1.55, gold: 0.65, drop: 1.28 },

        // OB34：傲慢之塔 51~60樓
        '受詛咒的妖魔殭屍':  { exp: 1.10, gold: 1.03, drop: 1.04 },
        '受詛咒的艾爾摩士兵':{ exp: 1.12, gold: 1.04, drop: 1.05 },
        '受詛咒的艾爾摩法師':{ exp: 1.15, gold: 1.05, drop: 1.06 },
        '受詛咒的艾爾摩將軍':{ exp: 1.18, gold: 1.06, drop: 1.07 },

        // 原始金幣 10000~20000，故額外壓低金幣倍率。
        '不死的木乃伊王':    { exp: 1.58, gold: 0.62, drop: 1.28 },

        // OB35：傲慢之塔 61~70樓
        '暗黑萊肯':          { exp: 1.10, gold: 1.03, drop: 1.04 },
        '冷酷冰原老虎':      { exp: 1.12, gold: 1.04, drop: 1.05 },
        '火焰烈炎獸':        { exp: 1.14, gold: 1.05, drop: 1.06 },
        '火焰阿西塔基奧':    { exp: 1.18, gold: 1.06, drop: 1.07 },

        // 原始金幣 10000~20000，故額外壓低金幣倍率。
        '冷酷的艾莉絲':      { exp: 1.60, gold: 0.60, drop: 1.28 },

        // OB36：傲慢之塔 71~80樓
        '暗黑黑騎士':        { exp: 1.12, gold: 1.04, drop: 1.05 },
        '暗黑火焰戰士':      { exp: 1.14, gold: 1.05, drop: 1.06 },
        '暗黑火焰弓箭手':    { exp: 1.13, gold: 1.05, drop: 1.06 },
        '暗黑思克巴女皇':    { exp: 1.18, gold: 1.07, drop: 1.08 },

        // 原始金幣 10000~20000，故額外壓低金幣倍率。
        '闇黑的騎士范德':    { exp: 1.65, gold: 0.58, drop: 1.28 },

        // OB37：傲慢之塔 81~90樓
        '傲慢的潔尼斯女王':  { exp: 1.16, gold: 1.05, drop: 1.06 },
        '小幻象眼魔':        { exp: 1.18, gold: 1.06, drop: 1.07 },
        '馬昆斯吸血鬼':      { exp: 1.20, gold: 1.07, drop: 1.08 },
        '恐怖的殭屍王':      { exp: 1.20, gold: 1.07, drop: 1.08 },

        // 原始金幣 10000~20000，故額外壓低金幣倍率。
        '不滅的巫妖':        { exp: 1.70, gold: 0.55, drop: 1.30 },

        // OB38：傲慢之塔 91~100樓
        '土精靈王':          { exp: 1.18, gold: 1.05, drop: 1.06 },
        '水精靈王':          { exp: 1.18, gold: 1.05, drop: 1.06 },
        '風精靈王':          { exp: 1.18, gold: 1.05, drop: 1.06 },
        '火精靈王':          { exp: 1.18, gold: 1.05, drop: 1.06 },
        '艾莉絲':            { exp: 1.20, gold: 1.06, drop: 1.07 },
        '木乃伊王':          { exp: 1.22, gold: 1.07, drop: 1.08 },
        ...
        // OB42：侵蝕的安塔瑞斯副本一般怪
        '喀瑪南':                    { exp: 1.05, gold: 1.00, drop: 1.00 },
        '喀瑪焰':                    { exp: 1.06, gold: 1.00, drop: 1.00 },
        '大地荒龍':                  { exp: 1.15, gold: 0.95, drop: 1.05 },

        // 三個區域頭目
        '喀瑪焰王':                  { exp: 1.25, gold: 0.85, drop: 1.15 },
        '喀瑪南王':                  { exp: 1.25, gold: 0.85, drop: 1.15 },
        '喀瑪王':                    { exp: 1.35, gold: 0.80, drop: 1.18 },

        // 最終三段變身：前兩段不死亡、不結算一般擊殺獎勵；
        // 設定仍保留，避免未來有其他生成入口時失去平衡。
        '被侵蝕的安塔瑞斯':          { exp: 1.45, gold: 0.70, drop: 1.20 },
        '被侵蝕的狂怒安塔瑞斯':      { exp: 1.60, gold: 0.62, drop: 1.24 },

        // 最終階原始金幣15000~25000；每日一次仍避免變成高額金幣來源。
        '被侵蝕的瘋狂安塔瑞斯':      { exp: 1.85, gold: 0.55, drop: 1.30 }
    };
...
/* ===== 仿正服 OB53：寵物升級需求稽核 START ===== */
(function officialPetExpRequirementAuditOB53(){
    if (!window.OFFICIAL_BALANCE_MODE || window.__officialPetExpRequirementAuditOB53) return;
    window.__officialPetExpRequirementAuditOB53 = true;

    const PETREQ53 = {
        officialPetExpReqDivisor: 4,
        originalPetExpReqDivisor: 10,

        // 需求提高時保留當級百分比，不把已累積努力直接清掉。
        preserveCurrentLevelProgressPct: true,

        petExpReqVersion: 4,

        // 前一版規則完整保留。
        ob52SharedPetExpPoolKept: true,
        downedPetsExcluded: true,
        playerLevelCapKept: true,
    ...
    setTimeout(function(){
        try {
            window.OFFICIAL_PET_EXP_AUDIT.runtimeOB53 = {
                petExpReq: typeof petExpReq === 'function',
                petExpReqDivisor:
                    typeof petExpReqDivisor === 'function'
                        ? petExpReqDivisor()
                        : null,
                petExpReqVersion:
                    typeof PET_EXP_REQ_VERSION !== 'undefined'
                        ? PET_EXP_REQ_VERSION
                        : null
            };

            console.info(
                '[official-OB53] pet EXP requirement audit',
                window.OFFICIAL_PET_EXP_AUDIT
            );
        } catch (e) {}
    }, 0);
